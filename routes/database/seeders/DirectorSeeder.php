...
class DirectorSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $filePath = str_replace('/', DIRECTORY_SEPARATOR, database_path('seeders/data/directors.txt'));
        if (!File::exists($filePath)) {
            $filePath = str_replace('/', DIRECTORY_SEPARATOR, database_path('seeders/data/directors.csv'));
            if (!File::exists($filePath)) {
        ...
        foreach ($lines as $line) {
            ...
            if (count($rowsBatch) >= $batchSize) {
                DB::table('directors')->insert($rowsBatch);
                $rowsBatch = [];
            }
             $progressBar->advance();
        }
