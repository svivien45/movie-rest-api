...
class ActorSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $filePath = str_replace('/', DIRECTORY_SEPARATOR, database_path('seeders/data/actors.txt'));
        if (!File::exists($filePath)) {
            $filePath = str_replace('/', DIRECTORY_SEPARATOR, database_path('seeders/data/actors.csv'));
            if (!File::exists($filePath)) {
        ...
        foreach ($lines as $line) {
            ...
            if (count($rowsBatch) >= $batchSize) {
                DB::table('actors')->insert($rowsBatch);
                $rowsBatch = [];
            }
             $progressBar->advance();
        }
