...
class StudioSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $filePath = str_replace('/', DIRECTORY_SEPARATOR, database_path('seeders/data/studios.txt'));
        if (!File::exists($filePath)) {
            $filePath = str_replace('/', DIRECTORY_SEPARATOR, database_path('seeders/data/studios.csv'));
            if (!File::exists($filePath)) {
        ...
        foreach ($lines as $line) {
            ...
            if (count($rowsBatch) >= $batchSize) {
                DB::table('studios')->insert($rowsBatch);
                $rowsBatch = [];
            }
             $progressBar->advance();
        }
