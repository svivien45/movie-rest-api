...
class CategorySeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $filePath = str_replace('/', DIRECTORY_SEPARATOR, database_path('seeders/data/categories.txt'));
        if (!File::exists($filePath)) {
            $filePath = str_replace('/', DIRECTORY_SEPARATOR, database_path('seeders/data/categories.csv'));
            if (!File::exists($filePath)) {
        ...
        foreach ($lines as $line) {
            ...
            if (count($rowsBatch) >= $batchSize) {
                DB::table('categories')->insert($rowsBatch);
                $rowsBatch = [];
            }
             $progressBar->advance();
        }
